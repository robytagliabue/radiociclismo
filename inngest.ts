...
// Helper function for manual-only workflow triggers (no automatic schedule)
export function registerManualTrigger(workflow: any) {
  console.log("🖐️ [registerManualTrigger] Registering manual trigger only", {
    workflowId: workflow?.id,
  });

  const manualFunction = inngest.createFunction(
    { id: "manual-trigger", concurrency: { limit: 1 } },
    { event: "replit/cron.trigger" },
    async ({ step }) => {
      return await step.run("execute-manual-workflow", async () => {
        console.log("🚀 [Manual Trigger] Starting manually triggered workflow", {
          workflowId: workflow?.id,
        ...
        try {
          ...
          console.log("📝 [Manual Trigger] Workflow run created", { runId: run?.runId });

          const result = await inngest.send({
            name: `workflow.${workflow.id}`,
            data: { runId: run?.runId, inputData: {} },
          });
          console.log("✅ [Manual Trigger] Invoked Inngest function", {
            workflowId: workflow?.id,
            runId: run?.runId,
          });
          return result;
        } catch (error) {
          console.error("❌ [Manual Trigger] Workflow execution failed", {
            workflowId: workflow?.id,
  ...
  console.log("✅ [registerManualTrigger] Manual trigger registered (no cron schedule)");
}
...
export function inngestServe({
  ...
  const serveHost = "http://localhost:5000";
